...
public class IntList {
    ...
    static IntList subTail(IntList L, int start) {
        ...
        for (int i = 0; i < start; i++) {
            finalList.head = finalList.tail.head;
            finalList.tail = finalList.tail.tail;
        }
        return finalList;
    }
    ...
    static IntList sublist(IntList L, int start, int len) {
        /* Your code here */
        IntList cList = new IntList();
        IntList starting;
        if (start >= 0 && len >= 0) {
        	starting = subTail(L, start);
        	cList = IntList.list();
        	while (len > 0) {
        		cList.head = starting.head;
            	starting.head = starting.tail.head;
            	starting.tail = starting.tail.tail;
            	len--;
          	}
        }
        return cList;
