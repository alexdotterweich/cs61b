...
class Arrays {
    ...
    static int[] catenate(int[] A, int[] B) {
        ...
        System.arraycopy(B, 0, fin, A.length, B.length);
        return fin;
    }
    ...
    static int[] remove(int[] A, int start, int len) {
        /* *Replace this body with the solution. */
        int[] fin = new int[A.length - len];
        System.arraycopy(A, 0, fin, 0, start);
        System.arraycopy(A, start + len, fin, start, A.length - (len + start));
        return fin;
    }
