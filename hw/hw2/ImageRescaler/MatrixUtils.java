 ...
 *
 *  @author Josh Hug and Alexandra Dotterweich
 */
...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        ...
        for (int i = 1; i < m[0].length; i++) {
            for (int q = 0; q < m.length; q++) {
                if (q == 0) {
                    result[i][q] += Math.min(result[i - 1][q], result[i - 1][q + 1]);
                } else if (q == m.length - 1) {
                    result[i][q] += Math.min(result[i - 1][q], result[i - 1][q - 1]);
                } else {
                    minVal = Math.min(result[i - 1][q], result[i - 1][q + 1]);
                    result[i][q] += Math.min(minVal, result[i - 1][q - 1]);
                }
            }
        }
        return result;
    ...
    /** public static double isValid(double[][] m, int r, int c) {
        if (r > 0 && c > 0 && m.length > r && m[0].length > c) {
            return m[r][c];
        } else {
