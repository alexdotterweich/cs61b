...
abstract class Board extends Observable {
    ...
    /** Returns the total number of blue squares on the board. */
    abstract int blueCountReturn();

    /** Returns the total number of red squares on the board. */
    abstract int redCountReturn();
    ...
    boolean isLegal(Side player, int n) {
        Square current = this.get(n);
        if (current.getSide() == player || current.getSide() == WHITE) {
            return true;
        }
        return false;
    }
    ...
    final Side getWinner() {
        if (redCountReturn() >= Math.pow(this.size(), 2)) {
            return RED;
        }
        if (blueCountReturn() >= Math.pow(this.size() , 2)) {
            return BLUE;
        } else {
            return null;
        }
    ...
    public String toString() {
        String output = "==="  + "\n" + "   ";
        int divideBy = 1;
        for (int r = 0; r < Math.pow(this.size(), 2); r += 1) {
            Square curr = this.get(r);
            int spots = curr.getSpots();
            ...
            if (player != WHITE) {
                output += (" " + spots + strPlayer);
            }
            if ((divideBy > 0) && ((divideBy / this.size())
                    != this.size()) && (divideBy % this.size() == 0)) {
                output += "\n" + "   ";
            }
            divideBy += 1;
        }
